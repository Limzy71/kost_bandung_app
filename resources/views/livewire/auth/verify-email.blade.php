<div class="w-full">
    ...
    <div class="bg-white border-4 border-black p-8 md:p-10 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] rounded-lg w-full">
        ...
        @if (session('status') == 'verification-link-sent')
            <div class="mb-6 flex items-center gap-3 bg-lime-300 border-4 border-black p-4 rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
        ...
        <div class="space-y-4">
            <form method="POST" action="{{ route('verification.send') }}">
                @csrf
                <button type="submit"
                    class="w-full py-4 px-6 bg-cyan-300 hover:bg-cyan-400 active:translate-x-1 active:translate-y-1 active:shadow-none text-black border-3 border-black font-black text-sm uppercase tracking-wider rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all flex items-center justify-center gap-2 cursor-pointer">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit"
                    class="w-full py-3 px-6 bg-white hover:bg-zinc-100 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none text-zinc-800 border-2 border-black font-black text-xs uppercase tracking-wider rounded-lg shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all flex items-center justify-center gap-2 cursor-pointer">
                    <x-icon name="lucide-log-out" class="w-4 h-4 text-black stroke-[2.5]" />
