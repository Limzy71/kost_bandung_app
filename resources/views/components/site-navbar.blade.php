...
<header class="bg-white border-b-3 border-black sticky top-0 z-50 shadow-[0_4px_0_0_rgba(0,0,0,1)] dark:bg-zinc-900 dark:border-zinc-700 dark:shadow-[0_4px_0_0_rgba(255,255,255,0.25)]"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-3">
        ...
        <nav class="hidden lg:flex items-center gap-2" aria-label="Navigasi utama">
            @auth
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                        class="inline-flex items-center gap-1.5 text-xs font-black uppercase text-black bg-rose-400 hover:bg-rose-300 px-3.5 py-2 border-2 border-black rounded shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none transition-all cursor-pointer dark:border-zinc-700 dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,0.25)]"
            ...
            @else
                @foreach ($guestItems as $item)
                    <a href="{{ $item['href'] }}" wire:navigate
                        class="inline-flex items-center gap-1.5 text-xs md:text-sm font-black uppercase text-black px-3.5 py-2 border-2 border-black rounded transition-all cursor-pointer hover:-translate-x-0.5 hover:-translate-y-0.5 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none dark:text-white dark:border-zinc-700 {{ $loop->first ? 'bg-white hover:bg-zinc-100 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] dark:bg-zinc-900 dark:hover:bg-zinc-800 dark:shadow-[3px_3px_0px_0px_rgba(255,255,255,0.25)]' : 'bg-yellow-400 hover:bg-yellow-300 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]' }}">
                        <x-icon name="{{ $item['icon'] }}" class="w-4 h-4 stroke-[2.5]" />
                        <span>{{ $item['label'] }}</span>
                    </a>
                @endforeach
        ...
        <button type="button" @click="open = !open" :aria-expanded="open" aria-controls="mobile-nav"
            class="lg:hidden inline-flex items-center justify-center w-10 h-10 shrink-0 bg-white hover:bg-zinc-100 border-2 border-black rounded shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none transition-all cursor-pointer dark:bg-zinc-900 dark:hover:bg-zinc-800 dark:border-zinc-700 dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,0.25)]"
    ...
    <div id="mobile-nav" x-show="open" x-cloak
        ...
        class="lg:hidden absolute top-full inset-x-0 bg-white border-b-4 border-black shadow-[0_4px_0_0_rgba(0,0,0,1)] max-h-[calc(100vh-4rem)] overflow-y-auto dark:bg-zinc-900 dark:border-zinc-700 dark:shadow-[0_4px_0_0_rgba(255,255,255,0.25)]">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4 space-y-2">
            @auth
                ...
                <form method="POST" action="{{ route('logout') }}" @click="open = false">
                    @csrf
                    <button type="submit"
                        class="w-full flex items-center gap-3 px-3.5 py-3 text-sm font-black uppercase text-black bg-rose-400 hover:bg-rose-300 border-2 border-black rounded transition-colors cursor-pointer dark:border-zinc-700">
            ...
            @else
                @foreach ($guestItems as $item)
                    <a href="{{ $item['href'] }}" wire:navigate @click="open = false"
                        class="flex items-center gap-3 px-3.5 py-3 text-sm font-black uppercase text-black border-2 border-black rounded transition-colors cursor-pointer dark:text-white dark:border-zinc-700 {{ $loop->first ? 'bg-white hover:bg-zinc-100 dark:bg-zinc-900 dark:hover:bg-zinc-800' : 'bg-yellow-400 hover:bg-yellow-300' }}">
                        <x-icon name="{{ $item['icon'] }}" class="w-5 h-5 shrink-0 stroke-[2.5]" />
                        <span>{{ $item['label'] }}</span>
                    </a>
                @endforeach
