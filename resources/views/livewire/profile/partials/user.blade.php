...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    <x-stat-card label="Total Pertanyaan" :value="$stats['totalInquiries']" hint="Pesan yang pernah Anda kirim" icon="lucide-message-square" color="bg-cyan-300" />
    <x-stat-card label="Belum Dibaca" :value="$stats['unreadInquiries']" hint="Pesan yang belum dibalas pemilik" icon="lucide-inbox" color="bg-pink-300" />
</div>
...
<div class="bg-white border-4 border-black rounded-2xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] overflow-hidden">
    <div class="bg-yellow-300 border-b-4 border-black px-6 py-4 flex items-center gap-3">
        ...
        <div>
            <h2 class="text-xl font-black text-black uppercase tracking-tight">Riwayat Pertanyaan Saya</h2>
            <p class="text-xs font-bold text-black/70">Daftar pesan yang telah Anda kirim ke pemilik kost.</p>
        </div>
    </div>

    <div class="divide-y divide-zinc-200">
        @forelse ($stats['inquiries'] as $inquiry)
            <div class="p-5 flex flex-col sm:flex-row sm:items-center gap-3">
                <div class="flex-1 min-w-0">
                    <div class="flex flex-wrap items-center gap-2">
                        @if ($inquiry->kost)
                            <a href="{{ route('kost.show', $inquiry->kost->slug) }}"
                    ...
                    <p class="text-xs font-bold text-zinc-600 mt-1 line-clamp-2">{{ $inquiry->message }}</p>
                </div>
                <div class="flex items-center gap-4 shrink-0">
                    @if ($inquiry->kost)
                        <span class="text-[10px] font-black uppercase text-zinc-500 inline-flex items-center gap-1">
                            ...
                            {{ $inquiry->kost->district }}
                        </span>
                    @endif
                    <span class="text-[10px] font-black uppercase text-zinc-500">{{ $inquiry->created_at?->translatedFormat('d M Y, H:i') }}</span>
                </div>
            </div>
        @empty
            <div class="p-10 text-center space-y-3">
                ...
                <p class="text-sm font-black uppercase text-zinc-500">Belum ada pertanyaan yang Anda kirim</p>
                <p class="text-xs font-bold text-zinc-500">Temukan kost impian Anda lalu kirim pertanyaan untuk mulai berkomunikasi dengan pemilik.</p>
                <a href="{{ route('home') }}"
