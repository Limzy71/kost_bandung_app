<div
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 space-y-8">
        ...
        <!-- Form Start -->
        <form wire:submit.prevent="save" x-data="{ formIsOutOfBounds: false }"
            @bounds-update.window="formIsOutOfBounds = $event.detail" class="space-y-8">
            ...
            <script>
                ...
                var DEFAULT_LAT = -6.917464,
                    DEFAULT_LNG = 107.619123;
                if (!window.kostMapInit) {
                    window.kostMapInit = function() {
                        return {
                            ...
                            ignoreNextAddressWatch: false,
                            markerManuallyMoved: false,
                            isGeocoding: false,
                            _updatingPosition: false,
                            geocodeMessage: '',
                            hasGoogleKey: '{{ $googleMapsApiKey }}',
                            get districtsData() {
                                return window.bandungDistricts || {};
                            },
                            ...
                            pickBestNominatim: function(results, query) {
                                ...
                                var isStreetLevel = function(cls, type) {
                                    ...
                                    return cls === 'highway' || cls === 'building' || cls === 'amenity' || cls ===
                                        'shop' ||
                                ...
                                for (var i = 0; i < results.length; i++) {
                                    if (isStreetLevel(results[i].class, results[i].type) && nameMatch(results[i]))
                                        return results[i];
                                }
                                for (var i = 0; i < results.length; i++) {
                                    if (isStreetLevel(results[i].class, results[i].type) && nameContains(results[i]))
                                        return results[i];
                                }
                                for (var i = 0; i < results.length; i++) {
                            ...
                            checkBounds: function(lat, lng) {
                                if (!this.district || !this.districtsData[this.district] || !this.districtsData[this
                                        .district].bounds) {
                                    this.isOutOfBounds = false;
                                    window.dispatchEvent(new CustomEvent('bounds-update', {
                                        detail: false
                                    }));
                                    return;
                                }
                                var b = this.districtsData[this.district].bounds;
                                this.isOutOfBounds = !(lat >= b.lat_min && lat <= b.lat_max && lng >= b.lng_min && lng <= b
                                    .lng_max);
                                window.dispatchEvent(new CustomEvent('bounds-update', {
                                    detail: this.isOutOfBounds
                                }));
                            },
                            ...
                            matchDistrict: function(components, source) {
                                var normalize = function(str) {
                                    if (!str) return '';
                                    return str.replace(/^kecamatan\s+/i, '').replace(/^kec\.?\s+/i, '').trim()
                                        .toLowerCase();
                                };
                                ...
                                if (source === 'google') {
                                    var levels = ['administrative_area_level_3', 'administrative_area_level_4',
                                        'sublocality_level_1', 'sublocality'
                                    ];
                                    for (var i = 0; i < levels.length; i++) {
                                        var comp = components.find(function(c) {
                                            return c.types.includes(levels[i]);
                                        });
                                        if (comp) {
                                            var match = checkAgainstKeys(comp.long_name) || checkAgainstKeys(comp
                                                .short_name);
                                            if (match) return match;
                                        }
                                    }
                                } else if (source === 'nominatim') {
                                    var fields = ['district', 'city_district', 'suburb', 'county', 'village', 'town',
                                        'municipality'
                                    ];
                            ...
                            updateDistrictFromMatch: function(matched) {
                                if (matched) {
                                    if (!this.district) {
                                        ...
                                        this.districtAutoMessage = 'Kecamatan disesuaikan: ' + this.district + ' ke ' +
                                            matched;
                                    }
                                    this.district = matched;
                                } else {
                                    this.districtAutoMessage =
                                        'Kecamatan tidak dapat dideteksi otomatis, silakan pilih manual.';
                                }
                                var lt = parseFloat(this.lat),
                                    ln = parseFloat(this.lng);
                            ...
                            reverseGeocode: function(lat, lng, isUserMapInteraction) {
                                ...
                                var useNominatimReverse = function() {
                                    fetch('https://nominatim.openstreetmap.org/reverse?format=json&lat=' + lat +
                                            '&lon=' + lng + '&addressdetails=1')
                                        .then(function(r) {
                                            return r.json();
                                        })
                                        .then(function(d) {
                                            if (d) {
                                                if (d.address) self.updateDistrictFromMatch(self.matchDistrict(d
                                                    .address, 'nominatim'));
                                    ...
                                    try {
                                        new google.maps.Geocoder().geocode({
                                            ...
                                            if (status === 'OK' && results && results[0]) {
                                                self.updateDistrictFromMatch(self.matchDistrict(results[0]
                                                    .address_components, 'google'));
                            ...
                            geocodeAddress: function(addr) {
                                ...
                                else if (self.district && !new RegExp(self.district.replace(/(\s)/g, '\\s'), 'i').test(q))
                                    q += ', ' + self.district;

                                var districtBounds = null;
                                if (self.district && self.districtsData[self.district] && self.districtsData[self.district]
                                    .bounds) {
                                ...
                                var useNominatim = function() {
                                    var url = 'https://nominatim.openstreetmap.org/search?format=json&q=' +
                                        encodeURIComponent(q) + '&limit=10&addressdetails=1';
                                    if (districtBounds) {
                                        url += '&viewbox=' + districtBounds.lng_min + ',' + districtBounds.lat_max +
                                            ',' + districtBounds.lng_max + ',' + districtBounds.lat_min;
                                    }
                                    fetch(url)
                                        .then(function(r) {
                                            return r.json();
                                        ...
                                        .then(function(d) {
                                            ...
                                            if (picked) {
                                                var rl = parseFloat(picked.lat),
                                                    rn = parseFloat(picked.lon);
                                                self.markerManuallyMoved = true;
                                                self.updateDistrictFromMatch(self.matchDistrict(picked.address,
                                                    'nominatim'));
                                ...
                                if (window.google && window.google.maps && window.google.maps.Geocoder) {
                                    ...
                                    if (districtBounds) {
                                        geocodeParams.bounds = new window.google.maps.LatLngBounds(
                                            new window.google.maps.LatLng(districtBounds.lat_min, districtBounds
                                                .lng_min),
                                            new window.google.maps.LatLng(districtBounds.lat_max, districtBounds
                                                .lng_max)
                                        );
                                    }
                                    var qClean = q.replace(/^jalan\s+/i, '').replace(/[^a-z0-9\s]/g, '').toLowerCase()
                                    ...
                                    try {
                                        new google.maps.Geocoder().geocode(geocodeParams, function(results, status) {
                                            ...
                                            if (status === 'OK' && results && results.length > 0) {
                                                var best = results[0],
                                                    bestScore = -1;
                                                for (var i = 0; i < results.length; i++) {
                                                    var addr = (results[i].formatted_address || '').replace(
                                                        /[^a-z0-9\s]/g, '').toLowerCase();
                                                    var wordMatch = qClean.length === 0 ? 0 : qClean.filter(
                                                        function(w) {
                                                            return addr.indexOf(w) !== -1;
                                                        }).length;
                                                    var s = (results[i].partial_match ? 0 : 10) + wordMatch * 3;
                                                    s += results[i].geometry.location_type === 'ROOFTOP' ? 5 :
                                                ...
                                                self.markerManuallyMoved = true;
                                                self.updateDistrictFromMatch(self.matchDistrict(best
                                                    .address_components, 'google'));
                                        ...
                                        console.warn('Google Geocoder threw, fallback to Nominatim', e);
                                        self.isGeocoding = false;
                                        useNominatim();
                                    }
                                } else {
                            ...
                            initMap: function() {
                                ...
                                var setupGoogle = function() {
                                    ...
                                    try {
                                        self.map = new google.maps.Map(self.$refs.mapElement, {
                                            center: {
                                                lat: lat0,
                                                lng: lng0
                                            },
                                        ...
                                        self.marker.addListener('dragend', function(e) {
                                            self.markerManuallyMoved = true;
                                            self.setCoords(e.latLng.lat(), e.latLng.lng(), true);
                                        });
                                ...
                                var setupLeaflet = function() {
                                    ...
                                    self.marker = L.marker([lat0, lng0], {
                                        draggable: true
                                    }).addTo(self.map);
                                    self.marker.on('dragend', function(e) {
                                        self.markerManuallyMoved = true;
                                        var p = e.target.getLatLng();
                                        self.setCoords(p.lat, p.lng, true);
                                    });
                                ...
                                this.$watch('lat', function() {
                                    if (!self._updatingPosition) self.updateMapPosition();
                                });
                                this.$watch('lng', function() {
                                    if (!self._updatingPosition) self.updateMapPosition();
                                });
                                this.$watch('district', function(newVal, oldVal) {
                                    var lt = parseFloat(self.lat),
                                        ln = parseFloat(self.lng);
                                    if (!isNaN(lt) && !isNaN(ln)) self.checkBounds(lt, ln);

                                    if (newVal && newVal !== oldVal && self.districtsData[newVal] && self
                                        .districtsData[newVal].center) {
                                        var center = self.districtsData[newVal].center;
                                        if (!self.markerManuallyMoved) {
                                            self.setCoords(center.lat, center.lng, false);
                                        } else if (self.map) {
                                            if (typeof L !== 'undefined' && self.map.setView) self.map.setView([
                                                center.lat, center.lng
                                            ], 14);
                                            else if (window.google && self.map.panTo) {
                                                self.map.panTo({
                                                    lat: center.lat,
                                                    lng: center.lng
                                                });
                ...
                <div class="flex flex-col sm:flex-row gap-6 items-start">
                    ...
                    <div class="space-y-2 flex-1 w-full min-w-0">
                        ...
                        <div class="relative w-full">
                            ...
                            <button type="button" x-show="address && !isGeocoding" x-cloak
                            ...
                            <div x-show="isGeocoding" x-cloak
                                class="absolute right-4 top-1/2 -translate-y-1/2 flex items-center justify-center text-zinc-400">
            ...
            <div class="flex items-center justify-end gap-4 pt-4 border-t-3 border-black">
                ...
                <button type="submit" wire:loading.attr="disabled" wire:target="save"
                    x-bind:disabled="formIsOutOfBounds"
                    :class="formIsOutOfBounds ? 'opacity-50 cursor-not-allowed bg-zinc-300' :
                        'bg-yellow-400 hover:bg-yellow-300 active:translate-x-1 active:translate-y-1 active:shadow-none'"
