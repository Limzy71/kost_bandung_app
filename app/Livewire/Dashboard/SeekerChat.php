...
class SeekerChat extends Component
{
    ...
    public function toggleArchive(int $id): void
    {
        ...
        $conversation->update([
            'status' => $conversation->isHiddenForSeeker()
                ? KostConversation::STATUS_OPEN
                : KostConversation::STATUS_ARCHIVED_BY_SEEKER,
        ]);
    ...
    {
        return KostConversation::where('seeker_id', Auth::id())->find($id);
    }

    protected function dispatchBadgeUpdate(): void
    {
        $count = KostMessage::whereNull('read_at')
            ->whereHas('conversation', fn ($q) => $q->where('seeker_id', Auth::id()))
            ->count();
    ...
    public function render(): View
    {
        $conversations = KostConversation::with(['kost', 'seeker', 'latestMessage'])
            ...
            ->where('status', '!=', KostConversation::STATUS_ARCHIVED_BY_SEEKER)
            ->latest('updated_at')
            ->get();
        ...
        return view('livewire.dashboard.seeker-chat', [
