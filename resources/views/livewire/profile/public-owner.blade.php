<div class="min-h-screen bg-[#f8f9fa] dark:bg-zinc-950 bg-[linear-gradient(to_right,#e5e7eb_1px,transparent_1px),linear-gradient(to_bottom,#e5e7eb_1px,transparent_1px)] bg-[size:24px_24px]">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 space-y-8">

        <!-- Back -->
        <div>
            <a href="{{ $backUrl }}"
                class="inline-flex items-center gap-2 bg-white dark:bg-zinc-900 text-black dark:text-white border-3 border-black dark:border-zinc-700 font-black text-xs uppercase px-4 py-2.5 rounded-xl shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] dark:shadow-[3px_3px_0px_0px_rgba(255,255,255,0.25)] hover:-translate-x-0.5 hover:-translate-y-0.5 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none transition-all cursor-pointer">
                ...
                <div class="flex-1 text-center sm:text-left space-y-2">
                    <div class="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                        ...
                        @if ($user->isIdentityVerified())
                            <span class="px-3 py-1 bg-emerald-400 text-black dark:text-white border-2 border-black dark:border-zinc-700 font-extrabold text-xs uppercase tracking-wider shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,0.25)] inline-flex items-center gap-1.5">
                                <x-icon name="lucide-badge-check" class="w-3.5 h-3.5 stroke-[2.5]" />
                                <span>Pemilik Terverifikasi</span>
                            </span>
                        @else
                            <span class="px-3 py-1 bg-rose-400 text-black dark:text-white border-2 border-black dark:border-zinc-700 font-extrabold text-xs uppercase tracking-wider shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,0.25)] inline-flex items-center gap-1.5" title="Identitas pemilik kost ini belum diverifikasi oleh admin">
                    ...
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 pt-2 text-sm">
                        <div class="flex items-center gap-2 text-zinc-700 dark:text-zinc-300 font-bold">
                            <x-icon name="lucide-user" class="w-4 h-4 text-black dark:text-white shrink-0 stroke-[2.5]" />
                            <span>{{ $user->name }}</span>
                        </div>
                        <div class="flex items-center gap-2 text-zinc-700 dark:text-zinc-300 font-bold">
                        ...
                        <div class="flex items-center gap-2 text-zinc-700 dark:text-zinc-300 font-bold">
                            <x-icon name="lucide-calendar" class="w-4 h-4 text-black dark:text-white shrink-0 stroke-[2.5]" />
        ...
        <div class="bg-white dark:bg-zinc-900 border-4 border-black dark:border-zinc-700 rounded-2xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] dark:shadow-[6px_6px_0px_0px_rgba(255,255,255,0.25)] overflow-hidden">
            <div class="bg-yellow-300 border-b-4 border-black dark:border-zinc-700 px-6 py-4 flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-xl font-black text-black dark:text-white uppercase tracking-tight">Daftar Kost</h2>
                    <p class="text-xs font-bold text-black dark:text-white/70">Kost yang sedang ditayangkan oleh pemilik ini.</p>
                </div>
            </div>
            ...
            <div class="divide-y divide-zinc-200">
                @forelse ($kosts as $kost)
                    <a href="{{ route('kost.show', $kost->slug) }}"
                        class="p-5 flex flex-col sm:flex-row sm:items-center gap-4 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors group">
                        ...
                        <div class="flex-1 min-w-0 space-y-1.5">
                            <div class="flex flex-wrap items-center gap-2">
                                <span class="font-black text-black dark:text-white uppercase text-sm group-hover:text-yellow-600 transition-colors truncate">{{ $kost->name }}</span>
                                <x-status-badge :status="$kost->is_available ? 'available' : 'full'" />
                                @if ($kost->isVerified())
                                    <span class="px-2 py-0.5 bg-emerald-400 text-black dark:text-white border-2 border-black dark:border-zinc-700 text-[10px] font-black uppercase rounded shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] dark:shadow-[1px_1px_0px_0px_rgba(255,255,255,0.25)] inline-flex items-center gap-1">
                                        <x-icon name="lucide-badge-check" class="w-3 h-3 stroke-[3]" />
                                        Terverifikasi
                                    </span>
                                @else
                                    <span class="px-2 py-0.5 bg-rose-400 text-black dark:text-white border-2 border-black dark:border-zinc-700 text-[10px] font-black uppercase rounded shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] dark:shadow-[1px_1px_0px_0px_rgba(255,255,255,0.25)] inline-flex items-center gap-1" title="Kost ini belum diverifikasi kepemilikannya">
                        ...
                        <div class="shrink-0">
                            <span class="inline-flex items-center gap-1.5 bg-black text-yellow-300 hover:bg-zinc-800 border-2 border-black dark:border-zinc-700 font-black text-xs uppercase px-3.5 py-2 rounded-xl shadow-[3px_3px_0px_0px_rgba(255,255,255,0.4)] group-hover:-translate-x-0.5 group-hover:-translate-y-0.5 transition-all">
                ...
                @empty
                    <div class="p-10 text-center space-y-3">
                        ...
                        <p class="text-sm font-black uppercase text-zinc-500 dark:text-zinc-400">Belum ada kost yang ditayangkan</p>
                        <p class="text-xs font-bold text-zinc-500 dark:text-zinc-400">Pemilik ini belum memiliki kost yang sedang tayang publik.</p>
                    </div>
                @endforelse
            </div>
        </div>
